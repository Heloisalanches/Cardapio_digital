...
// ---------- FOTO DO PRODUTO (escolhida do celular/PC, sem link) ----------
// Redimensiona e comprime a imagem no próprio navegador antes de salvar,
// para caber sem custo no Firestore (sem precisar do Cloud Storage pago).
...
let imagemSelecionada = null; // guarda a foto processada (base64) do formulário de adicionar

document.getElementById("campoImagemArquivo").addEventListener("change", async (e) => {
  ...
  imagemSelecionada = await processarImagem(arquivo);
...
document.getElementById("formProduto").addEventListener("submit", async (e) => {
  ...
  await db.collection("produtos").add({
    ...
    preco,
    categoria,
    imagemUrl: imagemSelecionada || null,
    disponivel: true,
...
function criarFormEdicao(p) {
  const id = p.id;
  let imagemEditada = p.imagemUrl || null; // mantém a foto atual até escolherem uma nova
  ...
  form.innerHTML = `
    ...
    <div class="campo-imagem">
      <label class="rotulo-imagem">Trocar foto (opcional)</label>
      <input type="file" class="edit-imagem-arquivo" accept="image/*" />
      <img class="preview-imagem-edicao" src="${escapeAttr(p.imagemUrl || "")}" style="${p.imagemUrl ? "" : "display:none;"}" alt="Pré-visualização" />
    </div>
  ...
  const previewEdicao = form.querySelector(".preview-imagem-edicao");
  form.querySelector(".edit-imagem-arquivo").addEventListener("change", async (e) => {
    const arquivo = e.target.files[0];
    if (!arquivo) return;
    imagemEditada = await processarImagem(arquivo);
    previewEdicao.src = imagemEditada;
